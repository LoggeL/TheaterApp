...
for(const file of files){
 ...
 const command=`import {unlinkSync} from 'node:fs';import {Store} from './src/store.mjs';const s=new Store('/data/theater.sqlite');const uid=${JSON.stringify(uid)},a=s.account(uid);if(a){const personId=a.personId;for(const m of s.all('media').filter(m=>m.ownerPersonId===personId)){try{unlinkSync('/data/media/'+m.id+'.webp');}catch(e){if(e.code!=='ENOENT')throw e;}s.delete('media',m.id);}for(const kind of ['responses','checkins','receipts','reminders','absences','devices','messages','pushJobs'])for(const r of s.db.prepare('SELECT id,data FROM entities WHERE kind=?').all(kind)){const d=JSON.parse(r.data);if(d.personId===personId||d.uid===uid||d.authorId===personId||(kind==='pushJobs'&&d.recipientPersonIds?.length===1&&d.recipientPersonIds[0]===personId))s.delete(kind,r.id);}s.delete('members',personId);s.db.prepare('DELETE FROM accounts WHERE uid=?').run(uid);s.db.prepare('DELETE FROM requests WHERE uid=?').run(uid);}s.close();`;
